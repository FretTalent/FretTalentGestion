...
export default function CandidateDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      {/* MODULE AUTO-CANDIDATURE PREMIUM (19,99 €) */}
      <div className="bg-white rounded-3xl border border-orange-200/80 shadow-md shadow-orange-500/5 p-6 sm:p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="bg-orange-500 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full shadow-xs">
                ⭐ Service Premium Chauffeur
              </span>
              {activeBadge && (
                <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-emerald-200">
                  ✓ Badge 48h Actif
                </span>
              )}
            </div>
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
                Pack Auto-Candidature Directe (Rayon 50 km)
              </h2>
              <p className="text-xs sm:text-sm text-slate-600 mt-1 leading-relaxed">
                Transmettez votre CV et dossier certifié en 1-clic à l&apos;ensemble des entreprises de transport situées dans un rayon de 50 km autour de votre domicile.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
              <div className="flex items-center gap-2 text-xs text-slate-700 font-medium">
                <span className="w-5 h-5 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center font-bold text-xs shrink-0">✓</span>
                <span>Envoi immédiat de vos documents certifiés</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-700 font-medium">
                <span className="w-5 h-5 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center font-bold text-xs shrink-0">✓</span>
                <span>Accusé d&apos;ouverture instantané par email</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-700 font-medium">
                <span className="w-5 h-5 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center font-bold text-xs shrink-0">✓</span>
                <span>Mise en avant prioritaire 48h sur la carte</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-slate-700 font-medium">
                <span className="w-5 h-5 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center font-bold text-xs shrink-0">✓</span>
                <span>Relance automatique programmée à J+7</span>
              </div>
            </div>

            {/* Suivi des candidatures existantes */}
            {myCandidatures.length > 0 && (
              <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 mt-3 space-y-1 text-xs">
                <div className="flex items-center justify-between font-bold text-slate-900">
                  <span>📊 Historique de votre dernière diffusion :</span>
                  <span className="text-orange-600 font-mono">
                    {new Date(myCandidatures[0].created_at).toLocaleDateString('fr-FR')}
                  </span>
                </div>
                <div className="flex items-center gap-4 text-slate-600 pt-1 flex-wrap font-mono">
                  <span>🏢 <strong>{myCandidatures[0].target_companies_count || myCandidatures[0].sent_count || 0}</strong> entreprises ciblées</span>
                  <span>👁️ <strong className="text-emerald-700">{myCandidatures[0].opened_count || 0}</strong> ouvertures confirmées</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="shrink-0 w-full lg:w-auto flex flex-col items-center lg:items-end gap-2">
            <div className="text-center lg:text-right">
              <span className="text-3xl sm:text-4xl font-black text-slate-900 font-mono">19,99 €</span>
              <span className="text-xs text-slate-400 block font-semibold">Paiement unique sans abonnement</span>
            </div>

            <button
              onClick={handlePurchasePremium}
              disabled={purchasingPremium}
              className="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-black text-sm rounded-2xl shadow-lg shadow-orange-500/25 transition-all transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2"
            >
              {purchasingPremium ? (
                <>
                ...
                <>
                  <span>🚀 Activer mon Auto-Candidature</span>
                  <span className="bg-white/20 px-2 py-0.5 rounded-lg text-xs">19,99 €</span>
                </>
              )}
            </button>
            <span className="text-[10px] text-slate-400">Paiement sécurisé par carte via Stripe</span>
          </div>
        </div>
      </div>
