...
export default function MissingDocuments({ candidateName, missingList }) {
  return (
    ...
    >
      ...
      <Text style={text}>
        Nous avons étudié votre dossier d'inscription sur FretTalent. Afin de pouvoir mettre en ligne votre profil et vous présenter aux recruteurs, <strong>certains documents obligatoires sont manquants ou non valides</strong>.
      </Text>
      ...
      <Section style={card}>
        ...
        <Hr style={hrSmall} />
        <ul style={list}>
          {missingList && missingList.map((doc, index) => (
            <li key={index} style={listItem}>{doc}</li>
          ))}
          {(!missingList || missingList.length === 0) && (
            <li style={listItem}>Veuillez vérifier votre espace candidat pour plus de détails.</li>
          )}
        </ul>
      </Section>

      <Text style={text}>
        Sans ces documents, votre profil restera invisible pour les entreprises. Merci de les télécharger au plus vite depuis votre espace personnel.
      </Text>
