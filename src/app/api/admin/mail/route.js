...
export async function POST(req) {
  try {
    const { target, specificEmails, type, subject, title, message, ctaText, ctaLink } = await req.json();
    ...
    if (target === "specific" && specificEmails) {
      recipientEmails = specificEmails.split(",").map(e => e.trim()).filter(e => e);
    } else if (target === "all_candidates" || target === "all_companies") {
      const tableName = target === "all_candidates" ? "candidates" : "companies";
      ...
      const roleFilter = target === "all_candidates" ? "candidate" : "recruiter";
