...
async function run() {
  ...
  try {
    ...
    // Supabase Auth stocke parfois la configuration de mailer dans auth.config
    // Mais explorons d'abord si nous pouvons modifier l'enregistrement ou appeler une fonction interne
    const res = await client.query("SELECT * FROM auth.config;");
    console.log('Lignes auth.config:', res.rows);
  } catch (err) {
    console.error('Erreur SQL Direct :', err.message);
  } finally {
    await client.end();
  }
}
