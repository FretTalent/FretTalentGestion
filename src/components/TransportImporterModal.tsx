            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
              <div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="🇫🇷 France entière (ou tapez 75, 59, 13, 02...)"
                    value={department}
                    disabled={isRunning}
                    onChange={(e) => setDepartment(e.target.value)}
                    className={`w-full px-3 py-2 border rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-slate-900/10 ${
                      !department ? 'bg-orange-50/60 border-orange-200 text-orange-950 placeholder:text-orange-700/60' : 'bg-white border-slate-200 text-slate-900'
                    }`}
                  />
                </div>
                <p className="text-[10px] text-slate-400 mt-1">
                  {!department ? '✓ Mode France Entière activé' : `Filtre actif sur le département ${department}`}
                </p>
              </div>
