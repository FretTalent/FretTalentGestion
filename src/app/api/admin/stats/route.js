...
// Helper pour récupérer toutes les lignes sans être bloqué par la limite PostgREST de 1000 lignes
async function fetchAllPageViews(supabaseAdmin, startDateIso, endDateIso = null) {
  let allRows = [];
  const pageSize = 1000;
  let from = 0;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    if (startDateIso && startDateIso !== new Date(0).toISOString()) {
    ...
    if (error) {
      console.error('Error in fetchAllPageViews:', error);
      break;
    }
    ...
    if (data && data.length > 0) {
      ...
      from += pageSize;
      if (allRows.length >= (count || 0) || data.length < pageSize) {
        hasMore = false;
      }
    } else {
      hasMore = false;
  ...
  return { rows: allRows, totalCount: exactTotalCount || allRows.length };
}
...
export async function GET(request) {
  try {
    ...
    // 1. Récupération paginée de toutes les vues (dépasse la limite 1000 PostgREST)
    const [
      currentResult,
      prevResult,
      ...
      fetchAllPageViews(supabaseAdmin, startDate.toISOString()),
      timeframe !== 'all'
        ? fetchAllPageViews(
            supabaseAdmin,
    ...
    const allViews = currentResult.rows || [];
    const previousPeriodViews = prevResult.rows || [];

    // 2. Calcul KPIs Globaux & Croissance vs Période N-1
    const totalViews = currentResult.totalCount || allViews.length;
    const prevTotalViews = prevResult.totalCount || previousPeriodViews.length;
    const viewsGrowth =
      prevTotalViews > 0
    ...
    const prevUniqueSessions = new Set(
      previousPeriodViews.map((v) => v.session_id),
    ).size;
    ...
    allViews.forEach((v) => {
      sessionViewsCount[v.session_id] =
        (sessionViewsCount[v.session_id] || 0) + 1;
    });
    const singlePageSessions = Object.values(sessionViewsCount).filter(
      (cnt) => cnt === 1,
    ).length;
    const estimatedBounceRate =
      uniqueSessions > 0
    ...
    // 4. Canaux d'Acquisition & SEO
    const channelStats = {
      organic: {
        label: 'Recherche Google / SEO',
        count: 0,
        color: '#10b981',
        icon: 'Search',
      },
      ...
      social: {
        label: 'Réseaux Sociaux (LinkedIn, etc.)',
        count: 0,
        color: '#0ea5e9',
        icon: 'Share2',
      },
      referral: {
        label: 'Sites Référents & Partenaires',
        count: 0,
        color: '#f59e0b',
        icon: 'ExternalLink',
      },
    ...
    allViews.forEach((v) => {
      ...
      if (
        ...
        const seName = ref.includes('google')
          ? 'Google'
          : ref.includes('bing')
          ? 'Bing'
          : 'Autre Moteur';
    ...
    allViews.forEach((v) => {
      ...
      if (
        p.includes('/tarifs') ||
        p.includes('/candidats-disponibles') ||
        p.includes('/offres') ||
        p.includes('/comment-ca-marche') ||
        p.includes('/chauffeurs') ||
        p.includes('/entreprises')
      ) {
    ...
    const newSignupsCount =
      (periodCandidates?.length || 0) + (periodCompanies?.length || 0);
    ...
    const funnel = {
      ...
      intentRate:
        uniqueSessions > 0
          ? Math.round((highIntentSessions.size / uniqueSessions) * 100)
          : 0,
      registerRate:
        uniqueSessions > 0
          ? Math.round((registerSessions.size / uniqueSessions) * 100)
          : 0,
      overallConversionRate:
        uniqueSessions > 0
          ? ((newSignupsCount / uniqueSessions) * 100).toFixed(1)
          : '0.0',
    };
    ...
    const topPages = Object.values(pageMap)
      .map((p) => ({
        ...
        organicPct:
          p.views > 0 ? Math.round((p.organicViews / p.views) * 100) : 0,
      }))
    ...
    const geoBreakdown = {
      france: {
        name: 'France',
        flag: '🇫🇷',
        candidatesCount: (periodCandidates || []).filter(
          (c) => c.country === 'FR',
        ).length,
      },
      belgium: {
    ...
    return NextResponse.json({
      ...
      kpis: {
        ...
        estimatedBounceRate,
        pagesPerSession:
          uniqueSessions > 0 ? (totalViews / uniqueSessions).toFixed(1) : '1.0',
      },
