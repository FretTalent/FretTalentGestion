      ...
      <main className="flex-grow max-w-5xl mx-auto px-4 py-16 sm:px-6 lg:px-8 w-full space-y-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <p className="text-slate-600">
            Trouvez les meilleures opportunités proches de chez vous. Postulez directement en partageant votre profil qualifié.
          </p>
        </div>
        ...
        {jobs.length === 0 ? (
          <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center text-slate-400 space-y-2">
            ...
            <p className="text-xs">Revenez très bientôt pour de nouvelles opportunités !</p>
          </div>
        ) : (
              ...
              >
                <div className="space-y-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-semibold text-slate-500">{job.companies?.name || "Entreprise Partenaire"}</p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-slate-500">
                    ...
                    <span className="flex items-center gap-1">
                      <Calendar className="h-4 w-4 text-slate-400" /> Publiée le {new Date(job.created_at).toLocaleDateString("fr-FR")}
                    </span>
                  </div>

                  <p className="text-sm text-slate-600 line-clamp-2 max-w-3xl">{job.description}</p>
                </div>
